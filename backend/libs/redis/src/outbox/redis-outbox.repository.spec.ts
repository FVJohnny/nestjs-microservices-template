...
describe('RedisOutboxRepository', () => {
  ...
  let repo: RedisOutboxRepository;
  ...
  beforeAll(async () => {
    ...
    repo = new RedisOutboxRepository(redisService);
  ...
  describe('processing state transitions', () => {
    it('moves events between unprocessed and processed indexes on save', async () => {
      ...
      // Verify event still exists in Redis but not in unprocessed index
      const client = redisService.getDatabaseClient()!;
      const storedEvent = await client.get('outbox:event:evt-transition');
      expect(storedEvent).toBeTruthy();
      const parsed = JSON.parse(storedEvent!);
      expect(parsed.processedAt).toBeTruthy();

      // Verify it's in the processed index
      const processedIds = await client.zrange('outbox:processedByProcessedAt', 0, -1);
      expect(processedIds).toContain('evt-transition');
    });
  ...
  describe('cleanup operations', () => {
    it('deletes only processed events older than cutoff date', async () => {
      ...
      expect(await client.get('outbox:event:evt-very-old')).toBeNull();
      expect(await client.get('outbox:event:evt-old')).toBeNull();

      // Recent and unprocessed should remain
      expect(await client.get('outbox:event:evt-recent')).not.toBeNull();
      expect(await client.get('outbox:event:evt-unprocessed')).not.toBeNull();
    ...
    it('handles edge case with exact cutoff time', async () => {
      ...
      // Events at or before cutoff should be deleted
      expect(await client.get('outbox:event:evt-exact')).toBeNull();
      expect(await client.get('outbox:event:evt-before')).toBeNull();

      // Event after cutoff should remain
      const afterEvent = await client.get('outbox:event:evt-after');
  ...
  describe('edge cases and error scenarios', () => {
    it('handles concurrent saves correctly', async () => {
      ...
      const client = redisService.getDatabaseClient()!;
      const stored = await client.get(`outbox:event:${eventId}`);
      expect(stored).toBeTruthy();

      // Should not be in unprocessed list since event2 is processed
      const unprocessed = await repo.findUnprocessed(10);
      expect(unprocessed.map((e) => e.id)).not.toContain(eventId);
    });
