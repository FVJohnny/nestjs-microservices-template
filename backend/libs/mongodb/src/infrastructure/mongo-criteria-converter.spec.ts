...
describe('MongoCriteriaConverter', () => {
  ...
  const createTestDocuments = (count: number, template?: Partial<TestDocument>): TestDocument[] => {
  ...
  describe('Empty Criteria', () => {
    ...
    it('should query database with empty criteria and return all documents with default pagination', async () => {
      ...
      const results = await MongoCriteriaConverter.query(collection, new Criteria()).toArray();
      
      expect(results).toHaveLength(3);
      expect(results.map(r => r.name).sort()).toEqual(['Item 1', 'Item 2', 'Item 3']);
    });
  });
  ...
  describe('PaginationOffset', () => {
    describe('Limit', () => {
      ...
      it('should query database with limit and return limited results', async () => {
        ...
        const criteria = new Criteria({ pagination: new PaginationOffset(3, 0) });
    ...
    it('should work with pagination and sorting combined', () => {
      const order = new Order(new OrderBy('value'), new OrderType(OrderTypes.DESC));
      const criteria = new Criteria({
        order,
      ...
      const result = MongoCriteriaConverter.convert(criteria);
      expect(result.filters).toEqual([]);
      expect(result.options).toEqual({ limit: 5, skip: 3, sort: { value: -1 } });
    });
    ...
    it('should generate consistent pagination options for dataset traversal', () => {
      ...
      for (let page = 0; page < totalPages; page++) {
        const criteria = new Criteria({ pagination: new PaginationOffset(pageSize, page * pageSize) });
    ...
    it('should handle pagination with complex criteria', () => {
      ...
      const order = new Order(new OrderBy('name'), new OrderType(OrderTypes.ASC));
      const criteria = new Criteria({
  ...
  describe('PaginationCursor', () => {
    ...
    it('should apply cursor pagination with cursor', () => {
      ...
      expect(result.filters[0].$or![1]).toEqual({ score: 30, id: { $gt: '3' } });
    });
    ...
    it('should handle cursor pagination with descending order', () => {
      ...
      expect(result.filters[0].$or![1]).toEqual({ score: 70, id: { $lt: '7' } });
    });
    ...
    it('should work with cursor pagination and filters combined', () => {
      ...
      expect(result.filters[1].$or![1]).toEqual({ score: 50, id: { $gt: '5' } });
    });
    ...
    it('should query database with cursor pagination and filters', async () => {
      ...
      expect(results).toHaveLength(2);
      expect(results.every(r => r.category === 'A' && r.active === true)).toBe(true);
  ...
  describe('Filters', () => {
    describe('Equal Filter', () => {
      it('should convert equal filter correctly', () => {
        const criteria = new Criteria({ filters: new Filters([createFilter('name', Operator.EQUAL, 'test')]) });
        const result = MongoCriteriaConverter.convert(criteria);
    ...
    const testFilterConversion = (field: string, operator: Operator, value: string, expectedFilter: object) => {
    ...
    it('should convert not equal filter', () => {
      testFilterConversion('status', Operator.NOT_EQUAL, 'inactive', { status: { $ne: 'inactive' } });
    });

    it('should convert contains filter', () => {
      testFilterConversion('description', Operator.CONTAINS, 'search', { description: { $regex: 'search', $options: 'i' } });
    });

    it('should convert not contains filter', () => {
      testFilterConversion('content', Operator.NOT_CONTAINS, 'spam', { content: { $not: { $regex: 'spam', $options: 'i' } } });
    });
    ...
    it('should convert less than filter', () => {
      testFilterConversion('price', Operator.LT, '100.50', { price: { $lt: 100.5 } });
    });
    ...
    it('should query database with not equal filter', async () => {
      ...
      const criteria = new Criteria({ filters: new Filters([createFilter('status', Operator.NOT_EQUAL, 'inactive')]) });
      const results = await MongoCriteriaConverter.query(collection, criteria).toArray();

      expect(results).toHaveLength(2);
      expect(results.every(r => r.status !== 'inactive')).toBe(true);
      expect(results.map(r => r.name).sort()).toEqual(['active-user', 'pending-user']);
    });
    ...
    it('should query database with contains filter', async () => {
      ...
      const criteria = new Criteria({ filters: new Filters([createFilter('title', Operator.CONTAINS, 'java')]) });
      const results = await MongoCriteriaConverter.query(collection, criteria).toArray();

      expect(results).toHaveLength(2);
      expect(results.map(r => r.title).sort()).toEqual(['Java Handbook', 'JavaScript Tutorial']);
    });
    ...
    it('should query database with greater than filter', async () => {
      ...
      const criteria = new Criteria({ filters: new Filters([createFilter('value', Operator.GT, '50')]) });
      const results = await MongoCriteriaConverter.query(collection, criteria).toArray();

      expect(results).toHaveLength(5);
      expect(results.every(r => r.value > 50)).toBe(true);
      expect(results.map(r => r.id).sort((a, b) => a - b)).toEqual([6, 7, 8, 9, 10]);
    });

    it('should query database with less than filter', async () => {
      const testDocs = [
        { product: 'Budget Item', price: 25.99 },
        { product: 'Mid Item', price: 75.50 },
        { product: 'Premium Item', price: 125.00 },
        { product: 'Cheap Item', price: 15.25 },
      ];
      await collection.insertMany(testDocs);
      
      const criteria = new Criteria({ filters: new Filters([createFilter('price', Operator.LT, '50')]) });
      const results = await MongoCriteriaConverter.query(collection, criteria).toArray();

      expect(results).toHaveLength(2);
      expect(results.every(r => r.price < 50)).toBe(true);
      expect(results.map(r => r.product).sort()).toEqual(['Budget Item', 'Cheap Item']);
    });
  ...
  describe('Sorting', () => {
    ...
    it('should convert descending sort', () => {
      const order = new Order(new OrderBy('createdAt'), new OrderType(OrderTypes.DESC));
      const criteria = new Criteria({ order });
      const result = MongoCriteriaConverter.convert(criteria);
      expect(result.filters).toEqual([]);
      expect(result.options).toEqual({ limit: 10, skip: 0, sort: { createdAt: -1 } });
    });
    ...
    it('should not add sort for none orderType', () => {
      const order = new Order(new OrderBy('name'), new OrderType(OrderTypes.NONE));
      const criteria = new Criteria({ order });
      const result = MongoCriteriaConverter.convert(criteria);
    ...
    it('should query database with descending sort', async () => {
      const testDocs = [
        ...
        { product: 'Widget B', price: 15.50, rating: 4.8 },
        { product: 'Widget C', price: 35.00, rating: 3.9 },
        { product: 'Widget D', price: 45.25, rating: 4.5 },
      ];
      await collection.insertMany(testDocs);
      
      const order = new Order(new OrderBy('price'), new OrderType(OrderTypes.DESC));
      const criteria = new Criteria({ order });
      const results = await MongoCriteriaConverter.query(collection, criteria).toArray();

      expect(results).toHaveLength(4);
      expect(results.map(r => r.price)).toEqual([45.25, 35.00, 25.99, 15.50]);
      expect(results.map(r => r.product)).toEqual(['Widget D', 'Widget C', 'Widget A', 'Widget B']);
    });
    ...
    it('should query database with sort and filters combined', async () => {
      ...
      const order = new Order(new OrderBy('salary'), new OrderType(OrderTypes.DESC));
      const criteria = new Criteria({
        filters: new Filters([createFilter('department', Operator.EQUAL, 'eng')]),
        order,
      });
      const results = await MongoCriteriaConverter.query(collection, criteria).toArray();
      ...
      expect(results.map(r => r.employee)).toEqual(['Tom', 'Jane', 'Alice', 'John']);
      expect(results.map(r => r.salary)).toEqual([95000, 92000, 88000, 85000]);
    });
  });
  ...
  describe('Count Method', () => {
    it('should count all documents when no filters applied', async () => {
      ...
      const count = await MongoCriteriaConverter.count(collection, new Criteria());
    ...
    it('should count filtered documents', async () => {
      ...
      const criteria = new Criteria({ filters: new Filters([createFilter('category', Operator.EQUAL, 'A')]) });
    ...
    it('should return 0 when no documents match filters', async () => {
      ...
      const criteria = new Criteria({ filters: new Filters([createFilter('category', Operator.EQUAL, 'C')]) });
      const count = await MongoCriteriaConverter.count(collection, criteria);
  ...
  describe('Complex Query Scenarios', () => {
    it('should query database with offset pagination, filters, and sorting', async () => {
      const testDocs = [
        { name: 'Alice', department: 'Engineering', salary: 95000, joinDate: new Date('2020-01-15') },
        { name: 'Bob', department: 'Engineering', salary: 87000, joinDate: new Date('2019-03-10') },
        { name: 'Charlie', department: 'Marketing', salary: 78000, joinDate: new Date('2021-06-20') },
        { name: 'Diana', department: 'Engineering', salary: 92000, joinDate: new Date('2020-08-05') },
        { name: 'Eve', department: 'Engineering', salary: 89000, joinDate: new Date('2018-11-30') },
        { name: 'Frank', department: 'Sales', salary: 76000, joinDate: new Date('2021-02-14') },
        { name: 'Grace', department: 'Engineering', salary: 98000, joinDate: new Date('2019-09-25') },
      ];
      await collection.insertMany(testDocs);
      
      const order = new Order(new OrderBy('salary'), new OrderType(OrderTypes.DESC));
      const criteria = new Criteria({
        filters: new Filters([createFilter('department', Operator.EQUAL, 'Engineering')]),
        order,
    ...
    it('should handle complex text search with multiple operators', async () => {
      const testDocs = [
        { title: 'Advanced JavaScript Patterns', content: 'Deep dive into JS', category: 'programming', views: 1500 },
        { title: 'Python for Beginners', content: 'Learn Python basics', category: 'programming', views: 2800 },
        { title: 'Marketing Strategies', content: 'Business growth tips', category: 'business', views: 900 },
        { title: 'JavaScript Testing Guide', content: 'Unit testing in JS', category: 'programming', views: 1200 },
        { title: 'Advanced Python Techniques', content: 'Pro Python tips', category: 'programming', views: 2100 },
      ];
      await collection.insertMany(testDocs);
      ...
      const order = new Order(new OrderBy('views'), new OrderType(OrderTypes.DESC));
      const criteria = new Criteria({ filters: new Filters(filters), order });
      const results = await MongoCriteriaConverter.query(collection, criteria).toArray();

      expect(results).toHaveLength(2);
      expect(results.every(r => r.category === 'programming' && r.views > 1000)).toBe(true);
      expect(results.map(r => r.title)).toEqual(['Advanced Python Techniques', 'Advanced JavaScript Patterns']);
    ...
    it('should handle large dataset with efficient pagination', async () => {
      ...
      const order = new Order(new OrderBy('value'), new OrderType(OrderTypes.ASC));
      const criteria = new Criteria({
      ...
      expect(results.map(r => r.value)).toEqual([710, 720, 730, 740, 750, 760, 770, 780, 790, 800]);
    });
  });
});