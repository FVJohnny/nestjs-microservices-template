...
import { INTEGRATION_EVENT_PUBLISHER_TOKEN, IntegrationEventPublisher } from '../integration-events';
...
export class OutboxService implements OnModuleInit, OnModuleDestroy {
  ...
  constructor(
    @Inject(OUTBOX_REPOSITORY_TOKEN) private readonly repository: OutboxRepository,
    @Inject(INTEGRATION_EVENT_PUBLISHER_TOKEN) private readonly publisher: IntegrationEventPublisher,
  ) {}
  ...
  private async handleEventError(event: OutboxEvent, error: unknown): Promise<void> {
    ...
    if (event.retryCount < event.maxRetries) {
      ...
      this.logger.warn(`Incremented retry count for event ${event.id}: ${event.retryCount + 1}/${event.maxRetries}`);
    } else {
