...
import { FilterOperator, Operator } from '../domain/criteria/filters/FilterOperator';
...
import { SharedAggregateRoot, SharedAggregateRootDTO } from '../domain/entities/AggregateRoot';
...
describe('InMemoryCriteriaConverter', () => {
  const createTestEntities = (count: number, template?: Partial<TestEntityDTO>): TestEntity[] => {
    return Array.from({ length: count }, (_, i) => {
      const baseData: TestEntityDTO = {
        ...
        tags: [`tag${i % 3 + 1}`, `category-${String.fromCharCode(65 + (i % 3))}`],
        ...template,
      };
  ...
  describe('Empty Criteria', () => {
    ...
    it('should query in-memory data with empty criteria', () => {
      ...
      expect(result.data.map(e => e.toValue().name).sort()).toEqual(['Alice', 'Bob', 'Charlie']);
  ...
  describe('Filters', () => {
    describe('Equal Filter', () => {
      it('should filter by equal condition', () => {
        ...
        expect(result.data.map(e => e.toValue().name).sort()).toEqual(['Alice', 'Charlie']);
      });

      it('should filter by boolean field', () => {
        const entities = createBasicTestEntities();
        const criteria = new Criteria({ filters: new Filters([createFilter('isActive', Operator.EQUAL, 'true')]) });
        
        const result = InMemoryCriteriaConverter.query(entities, criteria);
        
        expect(result.data).toHaveLength(2);
        expect(result.data.every(e => e.toValue().isActive === true)).toBe(true);
        expect(result.data.map(e => e.toValue().name).sort()).toEqual(['Alice', 'Bob']);
      });
    });
    ...
    describe('Not Equal Filter', () => {
      it('should filter by not equal condition', () => {
        ...
        expect(result.data).toHaveLength(2);
        expect(result.data.every(e => e.toValue().status !== 'inactive')).toBe(true);
        expect(result.data.map(e => e.toValue().name).sort()).toEqual(['Alice', 'Bob']);
      });
    });
    ...
    describe('Greater Than Filter', () => {
      it('should filter by greater than condition with numbers', () => {
        const entities = createBasicTestEntities();
        const criteria = new Criteria({ filters: new Filters([createFilter('age', Operator.GT, '27')]) });
        
        const result = InMemoryCriteriaConverter.query(entities, criteria);
        
        expect(result.data).toHaveLength(2);
        expect(result.data.every(e => e.toValue().age > 27)).toBe(true);
        expect(result.data.map(e => e.toValue().name).sort()).toEqual(['Bob', 'Charlie']);
      });

      it('should filter by greater than condition with dates', () => {
        const entities = createBasicTestEntities();
        const criteria = new Criteria({ filters: new Filters([createFilter('createdAt', Operator.GT, '2023-01-15')]) });
    ...
    describe('Less Than Filter', () => {
      it('should filter by less than condition', () => {
        const entities = createBasicTestEntities();
        const criteria = new Criteria({ filters: new Filters([createFilter('salary', Operator.LT, '80000')]) });
    ...
    describe('Contains Filter', () => {
      it('should filter by contains condition', () => {
        const entities = createBasicTestEntities();
        const criteria = new Criteria({ filters: new Filters([createFilter('email', Operator.CONTAINS, 'alice')]) });
      ...
      it('should be case insensitive for contains', () => {
        const entities = createBasicTestEntities();
        const criteria = new Criteria({ filters: new Filters([createFilter('name', Operator.CONTAINS, 'alice')]) });
    ...
    describe('Not Contains Filter', () => {
      it('should filter by not contains condition', () => {
        const entities = createBasicTestEntities();
        const criteria = new Criteria({ filters: new Filters([createFilter('email', Operator.NOT_CONTAINS, 'alice')]) });
    ...
    describe('Array Fields', () => {
      it('should handle array fields by joining them', () => {
        const entities = createBasicTestEntities();
        const criteria = new Criteria({ filters: new Filters([createFilter('tags', Operator.CONTAINS, 'senior')]) });
  ...
  describe('Sorting', () => {
    it('should sort in ascending order', () => {
      const entities = createBasicTestEntities();
      const order = new Order(new OrderBy('age'), new OrderType(OrderTypes.ASC));
      ...
      expect(result.data.map(e => e.toValue().name)).toEqual(['Alice', 'Charlie', 'Bob']);
    });
    ...
    it('should sort in descending order', () => {
      ...
      expect(result.data.map(e => e.toValue().name)).toEqual(['Bob', 'Alice', 'Charlie']);
    });
    ...
    it('should sort with filters combined', () => {
      ...
      expect(result.data).toHaveLength(2);
      expect(result.data.every(e => e.toValue().status === 'active')).toBe(true);
      expect(result.data.map(e => e.toValue().salary)).toEqual([85000, 75000]);
      expect(result.data.map(e => e.toValue().name)).toEqual(['Bob', 'Alice']);
    });
  });
  ...
  describe('PaginationOffset', () => {
    describe('Limit', () => {
      it('should limit results correctly', () => {
        ...
        expect(result.data).toHaveLength(3);
        expect(result.data.map(e => e.toValue().id)).toEqual(['id-1', 'id-2', 'id-3']);
        expect(result.total).toBeNull();
      });
    });
    ...
    describe('Combined Limit and Offset', () => {
      it('should apply both limit and offset', () => {
        ...
        expect(result.data).toHaveLength(3);
        expect(result.data.map(e => e.toValue().id)).toEqual(['id-6', 'id-7', 'id-8']);
      });
    });
    ...
    describe('WithTotal Flag', () => {
      it('should include total when withTotal is true', () => {
        const entities = createTestEntities(10);
        const criteria = new Criteria({ pagination: new PaginationOffset(3, 2, true) });
      ...
      it('should not include total when withTotal is false', () => {
        const entities = createTestEntities(10);
        const criteria = new Criteria({ pagination: new PaginationOffset(3, 2, false) });
  ...
  describe('PaginationCursor', () => {
    ...
    it('should apply cursor pagination with cursor (ascending)', () => {
      const entities = [
        ...
        new TestEntity({ id: 'id-3', name: 'Item 3', email: 'test@test.com', age: 20, score: 30, category: 'A', status: 'active', isActive: true, salary: 50000, department: 'Engineering', createdAt: new Date(), tags: [] }),
        new TestEntity({ id: 'id-4', name: 'Item 4', email: 'test@test.com', age: 20, score: 30, category: 'A', status: 'active', isActive: true, salary: 50000, department: 'Engineering', createdAt: new Date(), tags: [] }),
        new TestEntity({ id: 'id-5', name: 'Item 5', email: 'test@test.com', age: 20, score: 40, category: 'A', status: 'active', isActive: true, salary: 50000, department: 'Engineering', createdAt: new Date(), tags: [] }),
      ];
    ...
    it('should apply cursor pagination with cursor (descending)', () => {
      const entities = [
        ...
        new TestEntity({ id: 'id-3', name: 'Item 3', email: 'test@test.com', age: 20, score: 30, category: 'A', status: 'active', isActive: true, salary: 50000, department: 'Engineering', createdAt: new Date(), tags: [] }),
        new TestEntity({ id: 'id-2b', name: 'Item 2b', email: 'test@test.com', age: 20, score: 30, category: 'A', status: 'active', isActive: true, salary: 50000, department: 'Engineering', createdAt: new Date(), tags: [] }),
        new TestEntity({ id: 'id-5', name: 'Item 5', email: 'test@test.com', age: 20, score: 20, category: 'A', status: 'active', isActive: true, salary: 50000, department: 'Engineering', createdAt: new Date(), tags: [] }),
      ];
  ...
  describe('Complex Scenarios', () => {
    it('should handle filters + sorting + offset pagination', () => {
      ...
      const order = new Order(new OrderBy('age'), new OrderType(OrderTypes.DESC));
      const filters = [
      ...
      expect(result.data).toHaveLength(5);
      expect(result.data.every(e => e.toValue().department === 'Engineering')).toBe(true);
    ...
    it('should handle edge case with no matching results', () => {
      const entities = createBasicTestEntities();
      const criteria = new Criteria({ filters: new Filters([createFilter('category', Operator.EQUAL, 'Z')]) });
