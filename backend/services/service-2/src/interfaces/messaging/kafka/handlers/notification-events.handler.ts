...
export class NotificationEventsHandler implements KafkaTopicHandler {
  ...
  async handle(payload: KafkaMessagePayload): Promise<void> {
    ...
    try {
      ...
      this.logger.debug(`📧 Processing notification event [${messageId}]: ${JSON.stringify(data)}.  `);
      ...
      this.logger.log(`✅ Notification event [${messageId}] processed successfully in ${processingTime}ms.  `);

    } catch (error) {
      const processingTime = Date.now() - startTime;
      this.logger.error(`❌ Error processing notification-events message [${messageId}] after ${processingTime}ms: ${error}.  `);
  ...
  private async handleSendSMS(payload: any): Promise<void> {
    this.logger.log(`📱 Sending SMS to ${payload.phoneNumber} - Message: "${payload.message?.substring(0, 50)}...".  `);
  ...
  private async handlePushNotification(payload: any): Promise<void> {
    this.logger.log(`🔔 Sending push notification to ${payload.deviceId} - Title: "${payload.title}".  `);
