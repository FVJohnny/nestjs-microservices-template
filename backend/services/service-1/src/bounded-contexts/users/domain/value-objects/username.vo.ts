...
export class Username extends StringValueObject {
  ...
  static validate(username: string): void {
    ...
    if (trimmed.length < Username.MIN_LENGTH) {
      throw new DomainValidationException('username', trimmed, `Username must be at least ${Username.MIN_LENGTH} characters long`);
    }

    if (trimmed.length > Username.MAX_LENGTH) {
      throw new DomainValidationException('username', trimmed, `Username cannot exceed ${Username.MAX_LENGTH} characters`);
    }

    if (!Username.USERNAME_REGEX.test(trimmed)) {
      throw new DomainValidationException('username', trimmed, 'Username can only contain letters, numbers, underscores, and hyphens');
    }
  }
}