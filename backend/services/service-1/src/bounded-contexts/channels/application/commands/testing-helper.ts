...
export async function createTestingModule(options: CreateCQRSTestingModuleOptions) {
  ...
  if (eventHandlerOptions) {
    ...
    // Provide TestEventPublisher for INTEGRATION_EVENT_PUBLISHER_TOKEN when event handler is defined
  ...
  const integrationEventPublisher = eventHandlerOptions && moduleRef.get<IntegrationEventPublisher>(INTEGRATION_EVENT_PUBLISHER_TOKEN);
